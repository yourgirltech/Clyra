...
1. **Claim state** — a read-only snapshot:
   ...
   - `latest_recommendation` — `{ action_type, confidence, approval_status }` if one exists (`approval_status` ∈ `pending | approved | declined | none`)
...
| 12 | Trigger = `recommendation_completed` and confidence ≥ threshold | → **no_action** | `awaiting_human_approval` — surfaced in the UI for a human to approve/decline; Commander does not act further until it sees `human_approved` or `human_declined_action` |
| 13 | Trigger = `recommendation_completed` and confidence < threshold | → **06-escalation-agent** | `low_confidence` — a low-confidence recommendation is never shown as a one-click approval; see [03-recommendation-agent](./03-recommendation-agent.md) |
...
| 16 | Trigger = `followup_failed` (agent's own retries already exhausted) | → **06-escalation-agent** | `followup_execution_failed` |
| 17 | Trigger = `reminder_failed` (agent's own retries already exhausted) | → **06-escalation-agent** | `reminder_execution_failed` |
| 18 | Trigger = `followup_completed` or `reminder_completed` | → **no_action** | `action_executed` — log success, claim returns to steady state |
| 19 | Nothing above matched | → **06-escalation-agent** | `unclassified_trigger` — an event Commander doesn't recognize is never silently dropped |
