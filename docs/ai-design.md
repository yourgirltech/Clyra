...
## Status
Orchestration runtime: Python, `backend/app/agents/`. Commander (`commander.py`) and all five Phase-4-reachable agents are implemented: 01-analyzer (deterministic, no LLM) and 02-reasoning/03-recommendation (Claude API, structured output, grounded against their inputs) run in the claim-lifecycle pipeline via `dispatch.py`; 06-escalation (durable persistence, no LLM) is the real target of every non-happy-path rule; 07-assistant is the tool-calling chat agent behind the AI Assistant UI, invoked directly by `POST /api/ai/assistant`, outside Commander's routing table by design. 04-followup-agent and 05-reminder-agent remain Phase 6 work, unreachable by Commander's own design (see [00-commander](./agents/00-commander.md)). The AI Assistant chat UI is live end to end — no placeholders left in the Phase 4 build target. See each file in [`docs/agents/`](./agents/) for per-agent status.
